...
describe('user', () => {
    ...
    it('should register a user', async () => {
        const response = await request(app)
            ...
            .send({"name": "Menganito","age": 20,"email": "user29@test.com","password": "HolaMundo.01","city": "Madrid", "interests": ["cars"], "accept": "true", "role": "user"})
            .set('Accept', 'application/json')
            .expect(200)
        expect(response.body.user.name).toEqual('Menganito')
        expect(response.body.user.email).toEqual('user29@test.com')
